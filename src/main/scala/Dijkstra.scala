...
object Dijkstra {
  def findShortestPath(g: WeightedGraph[String]): Unit = {
    ...
    def shortestPath(step: ShortStep): ShortStep = {
      step
        .extractMin()
        .map { case (n, currentD) =>
          val newDists = g.neighboursWeights(n).collect {
            case WeightedEdge(m, w)
                if step.dists
                  .get(m)
                  .exists(_ > currentD + w) =>
              m -> (currentD + w)
          }
          val newParents = newDists.map { case (m, _) =>
            m -> n
          }
          shortestPath(
            ShortStep(
    ...
    def extractSPaths(
      ...
      parents
        .get(node)
        .map(p => node +: extractSPaths(p, parents))
        .getOrElse(List(node))
    }
